...
function TypingDots() {
    return (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, verticalAlign: 'middle', marginLeft: 4 }}>
            {[0, 1, 2].map((i) => (
                <span
                    key={i}
                    style={{
...
export default function ChatPanel({ user, userProfile, dbId, onClose }) {
    ...
    // Clear typing status when panel unmounts
    useEffect(() => {
    ...
    const typingLabel = typingUsers.length === 1
        ? `${typingUsers[0].toUpperCase()} IS TRANSMITTING`
        : `${typingUsers.length} AGENTS ARE TRANSMITTING`;
                        ...
                        {typingUsers.length > 0 && (
                            <div style={{
                                ...
                                <span style={{
                                    ...
                                    letterSpacing: '0.16em', color: 'var(--c-tx3)',
                                    textTransform: 'uppercase',
                                }}>
                                    {typingLabel}
                                </span>
