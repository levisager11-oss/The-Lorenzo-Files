...
import { AlertTriangle, Database, FileText, Upload, Loader2, Mail, RefreshCw, LogOut } from 'lucide-react';
  ...
  if (!user.emailVerified) {
    const handleResendVerification = async () => {
      try {
        ...
        alert('VERIFICATION LINK RE-TRANSMITTED. CHECK YOUR EMAIL.');
      } catch {
        alert('UNABLE TO SEND VERIFICATION. TRY AGAIN LATER.');
      }
    };
    ...
    const handleRefreshStatus = async () => {
      ...
      if (auth.currentUser.emailVerified) {
        // Force state update with a new object reference so React re-renders
        setUser({ ...auth.currentUser });
      } else {
        alert('EMAIL NOT YET VERIFIED. CHECK YOUR INBOX.');
      }
    };
